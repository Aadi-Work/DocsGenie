...
GEMINI_API_KEY = "AQ.Ab8RN6Knk_sRUsU08xegSgJcf5rQ0rZxAiPcMkQROt5IEJGqVQ"
...
# Initialize Google GenAI Client when the SDK is available
client = None
...
def ensure_mom_template_exists(
    ...
    doc.add_paragraph(
        "• Task: {{ item.task }} | Owner: {{ item.owner }} | Status: {{ item.status }}"
    )
    doc.add_paragraph("{% endfor %}")
...
def fetch_template_from_onedrive(
    template_name: str = DEFAULT_MOM_TEMPLATE,
) -> str:
    ...
    print(
        f"📡 '{template_name}' not found on local disk. Searching OneDrive cloud..."
    )
    try:
            ...
            for item in items:
                if item.get("name", "").lower() == template_name.lower():
                    ...
                    if file_res.status_code == 200:
                        ...
                        print(
                            f"✅ Successfully downloaded '{template_name}' from OneDrive into local workspace!"
                        )
                        return template_name
    except Exception as e:
    ...
    print(
        f"⚠️ Could not fetch '{template_name}' from OneDrive. Falling back to default generation..."
    )
    return ensure_mom_template_exists(template_name)
 
 
# ==============================================================================
# 4. VECTOR DB & INCREMENTAL ONEDRIVE SYNC (WITH VERBOSE FILE/FOLDER PRINTING)
# ==============================================================================
...
if embedding_functions is not None and chromadb is not None:
    try:
        ...
        print(
            f"⚠️ Chroma/SentenceTransformer initialization failed: {exc}. "
            "Vector search is disabled for this run."
        )
        embedding_fn = None
        chroma_client = None
        folder_collection = None
...
def sync_onedrive_to_vector_db(force_resync: bool = False):
    ...
    if force_resync:
        print(
            "🧹 Clearing local vector database and delta tokens for full resync..."
        )
        try:
    ...
    if os.path.exists(DELTA_TOKEN_FILE) and not force_resync:
        ...
        print(
            "⚡ Checking OneDrive for NEW, MODIFIED, or DELETED files/folders...\n"
        )
    else:
    ...
    while next_url:
        ...
        # Catch expired delta token (410 resyncRequired) and trigger full resync
        ...
        for item in items:
            ...
            if "folder" in item:
                folder_collection.upsert(
                    ...
                    metadatas=[
                        {"folder_name": item_name, "full_path": item_name}
                    ],
                )
                ...
                if file_res.status_code == 200:
                    parsed_text = ParserRegistry.parse(
                        file_res.content, item_name
                    )
                    chunks = chunk_text(parsed_text)
 
                    chunk_ids = [
                        f"{item_id}_chunk_{i}" for i in range(len(chunks))
                    ]
                    metadatas = [
                        {
                            "file_id": item_id,
...
def process_notes_with_gemini(raw_text: str) -> dict:
    """Uses Gemini JSON mode to extract discussion points and action items."""
    print("🧠 Extracting structured meeting summary with Gemini JSON mode...")
    start_time = time.perf_counter()
 
    prompt = f"""
    Read the following meeting notes. Do two things:
    1. Provide a brief summary of the discussion points as a list of strings.
    ...
    if client is None or types is None:
        summary = {
            "discussion_points": ["Meeting notes processed locally."],
            "action_items": [],
        }
...
def generate_meeting_minutes(
    ...
    # Automatically fetches template from OneDrive if missing locally
    resolved_template_path = fetch_template_from_onedrive(template_path)
 
    meeting_data = process_notes_with_gemini(raw_notes_text)
 
    print(
        f"📝 Rendering Word document with docxtpl using '{resolved_template_path}'..."
    )
    if DocxTemplate is None:
        raise RuntimeError("docxtpl is not installed. Install the document template dependency before generating the MOM.")
...
def find_folder_path(query: str) -> dict:
    ...
    try:
        for m in metadata[0]:
            folder_name = m.get("folder_name") if isinstance(m, dict) else None
            full_path = m.get("full_path") if isinstance(m, dict) else None
            if folder_name or full_path:
                matches.append({
    ...
    for i, meta in enumerate(metadata[0]):
        if isinstance(meta, dict):
            matches.append({
                ...
                "text_snippet": documents[0][i],
            })
    return {
        "status": "Content-only search results",
        "matching_documents": matches,
    }
 
 
def search_filename_and_content(
    query: str, find_latest_only: bool = False
) -> dict:
    ...
    for i, meta in enumerate(metadata[0]):
        ...
        if not f_id:
            continue
        file_name_value = meta.get("file_name", "")
        file_name = file_name_value.lower() if isinstance(file_name_value, str) else ""
        ...
        if f_id not in files_map:
            files_map[f_id] = {
                "file_id": f_id,
                "file_name": file_name_value if isinstance(file_name_value, str) else "Unknown",
                "created_date": meta.get(
    ...
    if find_latest_only:
        sorted_files = sorted(
            files_map.values(),
            key=lambda x: (x["score"], x["created_date"]),
            reverse=True,
        )
        return {
            "status": "Found latest document",
            "latest_file": sorted_files[0] if sorted_files else None,
        }

    sorted_files = sorted(
        files_map.values(), key=lambda x: x["score"], reverse=True
    )
    return {
        "status": "Hybrid search results",
        "matching_documents": sorted_files[:5],
    }
...
def summarize_and_generate_mom(
    ...
    file_name: str,
    project_name: str,
    date: str = "August 10, 2026",
) -> dict:
    """Reads OneDrive file content, extracts structured JSON summary, and populates Word template."""
    raw_text = read_file_content(file_id, file_name)
    return generate_meeting_minutes(
        raw_notes_text=raw_text, project_name=project_name, date=date
    )
...
1. If user asks specifically to search inside file text or content (e.g., "find files with content about X"):
   --> Use `search_content_only(query)`.
 
2. If user asks for a document by name or topic (e.g., "find MOM_KMM"):
...
3. If user asks for the "latest" or "newest" MOM/document on a topic:
   ...
   --> Present the identified file details (Name, Path, Created Date) and STOP to ask for confirmation:
       "Would you like me to summarize this document and generate a formatted Word document (.docx) using the MOM template? (Yes/No)"
 
WHEN USER CONFIRMS "YES":
1. Call `summarize_and_generate_mom(file_id, file_name, project_name, date)`.
2. Present the extracted discussion points, action items, and confirm file output path.
"""
 
 
def send_with_429_retry(
    chat, payload, max_retries: int = 5, pacing_seconds: float = 1.5
):
    """Sends requests to Gemini 3.6 Flash with dynamic spacing and exponential backoff on 429 rate limit errors."""
...
def send_message_and_handle_tools(chat, user_input: str):
    """Sends user input and automatically handles recursive tool execution calls."""
    if chat is None:
        return (
            "The Gemini chat client is unavailable in this environment. "
            "Install the Google GenAI SDK and rerun the assistant."
        )
...
def run_assistant():
    ...
    if chromadb is not None and embedding_functions is not None:
        ...
        print("⚠️ ChromaDB stack is unavailable in this environment. Local vector sync is skipped.")
 
    if client is None or types is None:
        print("⚠️ Google GenAI client is unavailable in this environment. The assistant will operate in a degraded mode.")
    ...
    if client is not None and types is not None:
        ...
        try:
            ...
            print("⚠️ Falling back to the first API-reported Gemini model for the current session.")
            try:
                available_models = list(client.models.list())
                if available_models:
                    fallback_model = getattr(available_models[0], "name", None)
                    if fallback_model:
                        MODEL_NAME = fallback_model.replace("models/", "")
                        chat = client.chats.create(model=MODEL_NAME, config=config)
            except Exception as low_exc:
                print(f"⚠️ Final fallback chat initialization failed: {low_exc}")
...
# ==============================================================================
# 2. BENCHMARKING & MS GRAPH AUTHENTICATION
# ==============================================================================
...
# ==============================================================================
# 4. STRICT TEMPLATE RESOLUTION (LOCAL -> CHROMADB -> ONEDRIVE)
# ==============================================================================
 
 
def fetch_template(template_name: str = DEFAULT_MOM_TEMPLATE) -> str:
    """Strict template resolution hierarchy:
    1. Local Workspace
    2. ChromaDB Vector Index
    3. OneDrive Cloud Search API
 
    Raises FileNotFoundError if the template cannot be found in any location.
    """
    ...
    print(
        f"🔍 [Tier 2] Searching ChromaDB index for template '{template_name}'..."
    )
    try:
        ...
        if (
            ...
            print(
                f"⚡ Found indexed template in ChromaDB (File ID: {file_id}). Fetching from OneDrive..."
            )
            token = acquire_microsoft_graph_token()
            headers = {"Authorization": f"Bearer {token}"}
            download_url = f"https://graph.microsoft.com/v1.0/me/drive/items/{file_id}/content"
            ...
            if res.status_code == 200:
                with open(template_name, "wb") as f:
                    f.write(res.content)
                print(
                    f"✅ Successfully downloaded '{template_name}' using ChromaDB metadata!"
                )
                return template_name
    except Exception as e:
        print(f"⚠️ ChromaDB lookup notice: {str(e)}")
 
    # Tier 3: OneDrive Cloud API Search
    print(
        f"📡 [Tier 3] Searching OneDrive Graph API directly for '{template_name}'..."
    )
    try:
        token = acquire_microsoft_graph_token()
        headers = {"Authorization": f"Bearer {token}"}
                ...
                if item.get("name", "").lower() == template_name.lower():
                    ...
                    if file_res.status_code == 200:
                        with open(template_name, "wb") as f:
                            f.write(file_res.content)
                        print(
                            f"✅ Successfully downloaded '{template_name}' from OneDrive cloud!"
                        )
                        return template_name
    except Exception as e:
        print(f"⚠️ Cloud search error: {str(e)}")
 
    # Strictly fail if not found across all 3 tiers
    raise FileNotFoundError(
        f"❌ Template '{template_name}' was not found in local directory, ChromaDB index, or OneDrive. "
        f"Please upload your '{template_name}' file to your workspace or OneDrive."
    )
 
 
# ==============================================================================
# 5. INCREMENTAL ONEDRIVE SYNC
# ==============================================================================
...
def sync_onedrive_to_vector_db(force_resync: bool = False):
    ...
    if os.path.exists(DELTA_TOKEN_FILE) and not force_resync:
        ...
        next_url = "https://graph.microsoft.com/v1.0/me/drive/root/delta?$select=id,name,createdDateTime,lastModifiedDateTime,folder,file,parentReference,deleted%22
        print("🔄 Performing initial OneDrive index...\n")
...
# ==============================================================================
# 6. GEMINI EXTRACTION & TOOL DEFINITIONS
# ==============================================================================
 
 
def process_notes_with_gemini(raw_text: str) -> dict:
    """Uses Gemini JSON mode to extract date, discussion points, and action items dynamically."""
    print("🧠 Extracting structured meeting summary with Gemini JSON mode...")
    start_time = time.perf_counter()
...
def generate_meeting_minutes(
    ...
    """Renders Word document using docxtpl and user-provided MOM template."""
    start_time = time.perf_counter()
 
    # Search local -> ChromaDB -> OneDrive API (Fails if missing)
    resolved_template_path = fetch_template(template_path)
 
    meeting_data = process_notes_with_gemini(raw_notes_text)
    final_date = (
        date if date else meeting_data.get("meeting_date", "Date Not Specified")
    )
...
def summarize_and_generate_mom(
    ...
    """Reads OneDrive file content, extracts date/summary via Gemini, and generates Word MOM."""
    raw_text = read_file_content(file_id, file_name)
    return generate_meeting_minutes(
        raw_notes_text=raw_text, project_name=project_name, date=date
    )
...
WHEN USER CONFIRMS "YES" OR REQUESTS MOM SUMMARY/GENERATION:
1. Call `summarize_and_generate_mom(file_id, file_name, project_name)`.
2. Present extracted discussion points, action items, and confirm document rendering status.
"""
...
def send_message_and_handle_tools(chat, user_input: str):
    """Sends user message and handles function tool invocations automatically."""
    response = send_with_429_retry(chat, user_input)
...
def run_assistant():
    if GEMINI_API_KEY == "YOUR_GEMINI_API_KEY_HERE":
        print(
            ...
            "Set environment variable GEMINI_API_KEY or Colab Secret before running."
        )
        return
