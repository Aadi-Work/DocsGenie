...
def acquire_microsoft_graph_token() -> str:
    ...
    if is_running_in_streamlit():
        ...
        ui_placeholder.warning(f"🔑 **Microsoft Authentication Required**\n\n1. Open link: [{verification_uri}]({verification_uri})\n2. Enter Code: **{user_code}**\n\n⏳ *Waiting for authorization in browser...*")
    elif COLAB_ENV:
        html_banner = f"""
    ...
    while time.time() - start_time < data.get("expires_in", 900):
        ...
        if "access_token" in token_data:
            if ui_placeholder:
                ui_placeholder.success("✅ Access token acquired successfully!")
            else:
...
# ==============================================================================
# 6. TOOL IMPLEMENTATIONS (All Original + New)
# ==============================================================================
...
def summarize_and_generate_mom(file_id: str, file_name: str, project_name: str = "Project Summary", doc_format: str = "excel") -> dict:
    """Reads OneDrive file content, extracts MOM via Gemini, and generates Excel or Word document."""
...
1. USER ASKS TO GENERATE MOM: Execute `search_filename_and_content` to find it, then `summarize_and_generate_mom`. You can pass doc_format="excel" or "word".
2. USER SEARCHES FOR SPECIFIC FILE BY NAME: Execute `search_filename_and_content`.
3. USER SEARCHES STRICTLY FOR IN-BODY TEXT/QUOTES: Execute `search_content_only`.
4. USER ASKS FOR FOLDER PATH LOCATIONS: Execute `find_folder_path`.
5. USER ASKS TO READ FULL FILE CONTENT: Find the file ID, then execute `read_file_content`.
6. USER ASKS FOR FILE HISTORY/CHART: Find the file ID, then execute `generate_version_history_chart`.
7. USER ASKS TO UPDATE OR EDIT TEXT FILE: Find the file ID, then execute `update_file_content`.
8. USER ASKS TO DELETE A FILE: Find the file ID, then execute `delete_file`.
...
def send_message_and_handle_tools(chat, user_input: str):
    ...
    while response.function_calls and api_call_count < MAX_API_CALLS_PER_TURN:
        for fn_call in response.function_calls:
            ...
            if is_running_in_streamlit():
                st.toast(f"⚙️ Executing: **{fn_name}**")
            else:
...
# ==============================================================================
# 7. FRONTEND (STREAMLIT & CLI DUAL BOOT)
# ==============================================================================
...
def run_streamlit_app():
    ...
    if "chat_session" not in st.session_state:
        config = types.GenerateContentConfig(system_instruction=SYSTEM_PROMPT, tools=list(TOOLS_MAP.values()))
        st.session_state.chat_session = client.chats.create(model=MODEL_NAME, config=config)

    st.title("☁️ OneDrive AI Assistant")

    if not st.session_state.user_email:
        st.subheader("Application User Portal Login")
        email_input = st.text_input("Enter your email address to establish permissions context:")
        
        if st.button("Login & Authorize"):
            if email_input:
                global USER_EMAIL
                USER_EMAIL = email_input.strip()
                st.session_state.user_email = USER_EMAIL
                
                st.info("Starting authentication flow... Please follow the Microsoft login instructions below if they appear.")
                try:
                    acquire_microsoft_graph_token()
                    with st.spinner("Syncing OneDrive..."):
                        sync_onedrive_to_vector_db(force_resync=FORCE_RESYNC_ON_START)
                    st.success("Successfully authenticated and synced!")
                    time.sleep(1)
                    st.rerun()
                except Exception as e:
    ...
    if prompt := st.chat_input("Ask about your files, generate MOM, delete, or update files..."):
        st.session_state.chat_history.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.markdown(prompt)
            
        with st.chat_message("assistant"):
            with st.spinner("Processing request and communicating with OneDrive..."):
...
def run_cli_assistant():
    """Original Terminal/CLI interface."""
    print("==================================================")
...
if __name__ == "__main__":
    if is_running_in_streamlit():
        ...
        print("\n💡 Tip: To run the web UI, use: `streamlit run script_name.py`\n")
